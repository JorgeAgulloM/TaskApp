...
sealed class TimeLimitAutoLogin {
    ...
    data class TwoWeeks(
        val time: Long = WEEK_IN_MILLIS * 2, val textTime: String = "two weeks"
    ) : TimeLimitAutoLogin()
    ...
    data class SixMonth(
        val time: Long = WEEK_IN_MILLIS * 28, val textTime: String = "Six month"
    ) : TimeLimitAutoLogin()

    data class OneYear(
        val time: Long = WEEK_IN_MILLIS * 56, val textTime: String = "One year"
    ) : TimeLimitAutoLogin()
}
